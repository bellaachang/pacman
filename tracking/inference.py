...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in range(0, X_RANGE):
        for j in range(0, Y_RANGE):
            pacman_possible.append((i, j))
            ghost0_possible.append((i, j))
            ghost1_possible.append((i, j))
    variableDomainsDict["Pacman"] = pacman_possible
    variableDomainsDict["Ghost0"] = ghost0_possible
    variableDomainsDict["Ghost1"] = ghost1_possible

    #For observations
    obs0_possible = list(range(0, X_RANGE + Y_RANGE + MAX_NOISE - 1))
    obs1_possible = list(range(0, X_RANGE + Y_RANGE + MAX_NOISE - 1))

    variableDomainsDict["Observation0"] = obs0_possible
    variableDomainsDict["Observation1"] = obs1_possible
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for h in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, h)
            if len(joinedFactor.unconditionedVariables()) != 1: 
                joinedFactor = eliminate(joinedFactor, h)
                currentFactorsList.append(joinedFactor)
            joint = joinFactors(currentFactorsList)
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance == None and ghostPosition == jailPosition:
            ...
            distance_between = manhattanDistance(pacmanPosition, ghostPosition)
            return busters.getObservationProbability(noisyDistance, distance_between)
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            for i in newPosDist.keys():
                copy[i] += (self.beliefs[pos] * newPosDist[i])
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in self.particles:
            weighted[p] += self.getObservationProb(observation, gameState.getPacmanPosition(), p, self.getJailPosition())
        weighted.normalize()
        ...
        if weighted.total() == 0:
            ...
            lol = []
            for w in range(len(self.particles)):
                lol.append(weighted.sample())
            self.particles = lol
        "*** END YOUR CODE HERE ***"
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        all_possible = list(itertools.product(self.legalPositions, self.legalPositions))
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                curr_weight *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), p[i], self.getJailPosition(i))
            weighted[p] += curr_weight
        weighted.normalize()
